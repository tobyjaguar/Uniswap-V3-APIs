...
async def test_chart_data_has_correct_structure():
    ...
    assert all(len(data_type) == 25 for data_type in respJSON), "Each category should have 25 data points"
...
async def test_chart_data_has_correct_data():
    ...
    for i, data_type in enumerate(respJSON):
        for point in data_type:
            assert len(point) == 3, f"Each data point in {data_types[i]} should have 3 elements"
            assert isinstance(point[0], str), f"Timestamp in {data_types[i]} should be a string"
            assert re.match(timestamp_pattern, point[0]), f"Timestamp should be in the format 'YYYY-MM-DDTHH:MM:SS'"
            assert isinstance(point[1], str), f"Second element should be a string"
            assert point[1] == data_types[i], f"Second element should be '{data_types[i]}'"
            assert isinstance(point[2], float), f"Value in {data_types[i]} should be a float"
            assert str(point[2]).split('.')[-1] <= '9', f"Float value not truncated to one decimal place: {point[2]}"
...
async def test_chart_data_has_correct_format():
    ...
    assert all(datetime.fromisoformat(ts) for ts in timestamps), "All timestamps should be in ISO format"
    # assert all(is_valid_iso_format(ts) for ts in timestamps), "All timestamps should be in ISO format"
    # assert is_chronological_order(timestamps), "Timestamps should be in chronological order"
